...
if __name__ == "__main__":
    ...
    for img_id, R in numpy_data.items():
        with tf.Session(config=config) as sess:
            ...
            number = int(re.search(r"(\d+)_gt\.json", args.target_json_file).group(1))

            img = dataset.render_rot( R,downSample = 1)
            base_dir = os.path.dirname(args.target_json_file)
            folder_dir = os.path.join(base_dir, f"{number}_gt")
            os.makedirs(folder_dir, exist_ok=True)
            img_path = os.path.join(folder_dir, f"{img_id}.png")
